...
import { editRentalItem, type ApiRentalItem } from "@/lib/rental-actions";
...
export default function EditRentalForm({ item }: { item: ApiRentalItem }) {
  ...
  const [pricePerDay, setPricePerDay] = useState(String(parseFloat(String(item.pricePerDay))));
  const [deposit, setDeposit] = useState(item.deposit ? String(parseFloat(String(item.deposit))) : "");
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        ...
        success("Item sewa berhasil diperbarui!");
    ...
    <form onSubmit={handleSubmit} noValidate>
      <header className="sticky top-0 bg-white/80 backdrop-blur-md z-30 px-6 py-4 border-b border-[#e1e3e4] flex justify-between items-center -mx-6 mb-6">
        <div>
          ...
          <h2 className="text-xl font-bold text-[#003527]">Edit Item Sewa</h2>
        </div>
