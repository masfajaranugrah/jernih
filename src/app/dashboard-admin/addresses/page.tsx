...
export default function AddressesPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] text-[#191c1d] antialiased">
      ...
      {/* TopNavBar */}
      <header className="bg-[#f8f9fa] w-full h-16 sticky top-0 shadow-sm z-40 flex items-center">
        <div className="flex justify-between items-center px-6 max-w-[1280px] mx-auto w-full">
          <div className="flex items-center gap-6">
            ...
            <nav className="hidden md:flex gap-6 ml-6">
              {["Shop", "Deals", "New Arrivals"].map((item) => (
                <a key={item} href="#" className="text-[#404944] text-sm font-semibold hover:text-[#003527] transition-colors">{item}</a>
              ))}
            </nav>
          </div>
          <div className="flex items-center gap-2">
            {["notifications", "shopping_cart", "account_circle"].map((icon) => (
              <button key={icon} className="p-2 text-[#404944] hover:text-[#003527] transition-colors">
      ...
      <main className="lg:ml-64 min-h-screen p-6 md:p-12 pb-28 lg:pb-12">
