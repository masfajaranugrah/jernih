...
import type { HeroData, HeroBanner, HeroMain } from "@/lib/hero-store";
...
function SideCardSlider({
  ...
  return (
    <div
      className={`group relative w-full overflow-hidden rounded-2xl sm:rounded-3xl shadow-sm border border-slate-100/60 ${className}`}
...
export default function HeroSectionClient({ hero }: { hero: HeroData }) {
  ...
  const mainSlides = [
    {
      id: "main-starlink",
      linkHref: main.linkHref || "/produk/starlink-v4",
      render: () => (
        <div className="relative flex h-full w-full flex-col justify-between overflow-hidden bg-[#060c18] p-6 sm:p-8 text-white">
          ...
          {main.imageUrl ? (
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-black via-slate-900 to-blue-950" />
          )}

          <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/30 to-transparent" />

          {/* Badge & Starlink Standard V4 Price Tag */}
          ...
          <div className="relative z-10 max-w-xl">
            <h1 className="text-2xl font-black leading-tight tracking-tight sm:text-4xl lg:text-4xl">
              {main.title || "STARLINK STANDARD V4"}
              {main.titleSuffix && (
                <span className="font-light italic text-sky-300"> {main.titleSuffix}</span>
              )}
            </h1>
            <p className="mt-2 max-w-md text-xs sm:text-sm text-white/80 line-clamp-2">
              {main.description || "Internet Satelit Kecepatan Tinggi untuk Rumah & Bisnis. Kuota Tanpa Batas dengan Latensi Rendah."}
            </p>
  ...
  const topCardSlides: SideSlideItem[] = [
    // Slide 1: Intel Powering Ideas Banner (Match Screenshot 1 & 2)
    {
      id: "top-intel-powering-ideas",
      linkHref: "/produk?category=laptop",
      renderContent: () => (
        <div className="relative flex h-full w-full flex-col justify-between overflow-hidden bg-gradient-to-br from-[#302111] via-[#1d1309] to-[#0d0702] p-3.5 sm:p-5 lg:p-6 text-white border border-amber-900/30">
          ...
          {/* Main Title (Flex Content Middle) */}
          ...
          {/* Bottom Flex Row (Badge + Dots) */}
          <div className="relative z-10 flex items-center justify-between gap-1.5 sm:gap-2 w-full mt-auto">
            <span className="shrink-0 rounded-full bg-[#facc15] px-2.5 sm:px-3.5 py-0.5 sm:py-1 text-[8px] sm:text-[11px] font-black uppercase text-black tracking-tight shadow-md select-none whitespace-nowrap">
              FREE SMARTWATCH
            </span>
          </div>
        </div>
    ...
    // Slide 2: Intel Gamer Days Banner
    {
      ...
      renderContent: () => (
        <div className="relative flex h-full w-full flex-col justify-between overflow-hidden bg-gradient-to-br from-[#030d30] via-[#081e62] to-[#020921] p-3.5 sm:p-5 lg:p-6 text-white border border-blue-900/30">
          ...
          <div className="relative z-10 flex items-center justify-between gap-1.5 w-full">
            <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
              ...
              <span className="rounded-sm bg-blue-600 px-1 sm:px-1.5 py-0.5 text-[8px] sm:text-[9px] font-black uppercase tracking-wider text-white">
                Gamer Days
              </span>
            </div>
            <div className="shrink-0 flex items-center gap-1 rounded-full bg-white/10 px-2 sm:px-3 py-0.5 sm:py-1 backdrop-blur-md border border-white/10">
              ...
              <span className="text-[8px] sm:text-xs font-bold uppercase tracking-wider text-white">AGRES.ID</span>
            </div>
          </div>
          ...
          <div className="relative z-10 my-auto py-0.5">
            <h3 className="text-xs sm:text-base lg:text-lg font-black leading-tight sm:leading-snug tracking-tight text-white">
              Promo terbaik <br />
              <span className="text-[#38bdf8]">di dunia game</span>
            </h3>
            <p className="mt-0.5 sm:mt-1 text-[9px] sm:text-xs leading-tight text-white/80 line-clamp-1">
              Dapatkan dua game dengan membeli Produk Intel® pilihan.
            </p>
          </div>

          {/* Bottom Flex Row (Badge + Dots) */}
          <div className="relative z-10 flex items-center justify-between gap-1.5 sm:gap-2 w-full mt-auto">
            <span className="shrink-0 rounded-full bg-[#facc15] px-2.5 sm:px-3.5 py-0.5 sm:py-1 text-[8px] sm:text-[11px] font-black uppercase text-black tracking-tight shadow-md whitespace-nowrap">
              FREE HEADSET GAMING
            </span>
          </div>
        </div>
    ...
    // Slide 3: AGRES CARE Protection
    {
      ...
      renderContent: () => (
        <div className="relative flex h-full w-full flex-col justify-between overflow-hidden bg-gradient-to-br from-[#850b12] via-[#5c0308] to-[#2b0003] p-3.5 sm:p-5 lg:p-6 text-white border border-rose-900/30">
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent pointer-events-none" />
          ...
          <div className="relative z-10 flex items-center justify-between gap-1.5 w-full">
            ...
            <span className="shrink-0 rounded-full bg-white/10 px-2 sm:px-3 py-0.5 sm:py-1 text-[8px] sm:text-xs font-bold text-amber-300 uppercase border border-white/10">
              1 TAHUN KAMI GANTI
            </span>
          </div>
          ...
          {/* Bottom Flex Row (Badge + Dots + Right Text) */}
          <div className="relative z-10 flex items-center justify-between gap-1 sm:gap-2 w-full mt-auto">
            <span className="shrink-0 rounded-full bg-rose-600 px-2 sm:px-3 py-0.5 sm:py-1 text-[8px] sm:text-[11px] font-extrabold text-white uppercase tracking-tight whitespace-nowrap">
              +2 TAHUN SERVIS
            </span>
            <span className="shrink-0 text-[8px] sm:text-xs text-amber-300 font-bold text-right hidden xs:inline-block">
              GRATIS 500RB
            </span>
          </div>
        </div>
  ...
  // If custom banner 0/1 exists in backend data, append or insert it
  if (b0 && b0.imageUrl) {
    topCardSlides.unshift({
      id: "top-custom-b0",
      linkHref: "/produk",
      renderContent: () => (
        <div className="relative flex h-full w-full flex-col justify-between overflow-hidden bg-slate-900 p-3.5 sm:p-5 text-white">
          <Image src={b0.imageUrl} alt={b0.title} fill className="object-cover object-center opacity-75" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/20 to-transparent" />
          <div className="relative z-10 flex justify-between items-start">
            <span className="bg-yellow-400 text-black text-[9px] sm:text-xs font-black uppercase px-2 sm:px-2.5 py-0.5 rounded-md">
              {b0.label || "PROMO"}
            </span>
          </div>
          <div className="relative z-10 my-auto">
            <h3 className="text-xs sm:text-base lg:text-lg font-black text-white leading-tight">{b0.title}</h3>
            {b0.subtitle && <p className="text-[9px] sm:text-xs text-white/80 line-clamp-1">{b0.subtitle}</p>}
          </div>
          <div className="relative z-10 flex items-center justify-between gap-2 w-full mt-auto">
            <span className="shrink-0 text-[9px] sm:text-xs font-bold text-white/80">AGRES.ID</span>
            <span className="shrink-0 text-[9px] sm:text-xs font-bold text-yellow-400">Promo</span>
          </div>
        </div>
      ),
    });
  ...
  const bottomCardSlides: SideSlideItem[] = [
    // Slide 1: Hisense Retailer Banner (Match Screenshot 1 & 3)
    {
      id: "bottom-hisense-official",
      linkHref: "/produk?category=tv",
      renderContent: () => (
        <div className="relative flex h-full w-full flex-col justify-between overflow-hidden bg-gradient-to-br from-[#4d0309] via-[#330105] to-[#1a0002] p-3.5 sm:p-5 lg:p-6 text-white border border-rose-950/40">
          <div className="absolute -right-8 -bottom-8 h-32 w-32 rounded-full bg-rose-500/10 blur-2xl pointer-events-none" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent pointer-events-none" />

          {/* Header Row (Flex Row) */}
          <div className="relative z-10 flex items-center justify-between gap-1.5 w-full">
            <span className="text-xl sm:text-2xl lg:text-3xl font-black italic tracking-tight text-[#00c594] select-none shrink-0">
              Hisense
            </span>
            <div className="flex items-center gap-1 sm:gap-2 shrink-0">
              <span className="text-[8px] sm:text-xs font-extrabold uppercase tracking-wider text-white bg-white/15 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full backdrop-blur-md border border-white/10 select-none">
                OFFICIAL RETAILER
              </span>
              <span className="text-[8px] sm:text-xs font-black tracking-wide text-amber-400 bg-black/60 px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-md border border-amber-400/20 select-none">
                AGRES.ID
              </span>
            </div>
          </div>
          ...
          {/* Bottom Flex Row (Badge + Dots + Right Text) */}
          <div className="relative z-10 flex items-center justify-between gap-1.5 sm:gap-2 w-full mt-auto">
            <span className="shrink-0 rounded-full bg-[#e11d48] px-2.5 sm:px-3.5 py-0.5 sm:py-1 text-[8px] sm:text-[11px] font-black uppercase text-white tracking-tight shadow-md select-none whitespace-nowrap">
              PROMO AGUSTUS 2026
            </span>
            <span className="shrink-0 text-[9px] sm:text-xs font-bold text-[#2dd4bf] tracking-tight select-none hidden xs:inline-block whitespace-nowrap">
              Garansi Resmi
            </span>
          </div>
        </div>
    ...
    // Slide 2: Hisense Appliances Showcase
    {
      ...
      renderContent: () => (
        <div className="relative flex h-full w-full flex-col justify-between overflow-hidden bg-gradient-to-br from-[#1a2e23] via-[#0d1a13] to-[#040a07] p-3.5 sm:p-5 lg:p-6 text-white">
          ...
          <div className="relative z-10 flex items-center justify-between gap-1.5 w-full">
            ...
            <span className="shrink-0 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 px-2 sm:px-3 py-0.5 sm:py-1 text-[8px] sm:text-xs font-bold">
              #1 Global Brand
            </span>
          </div>
          ...
          <div className="relative z-10 my-auto py-0.5">
            <h3 className="text-xs sm:text-base font-black text-white leading-tight">
              Diskon Elektronik Rumah <span className="text-emerald-400">Hingga 40%</span>
            </h3>
            <p className="mt-0.5 sm:mt-1 text-[8px] sm:text-xs text-white/80 line-clamp-1">
              Gratis Pengiriman & Pemasangan
            </p>
          </div>

          {/* Bottom Flex Row (Badge + Dots + Right Text) */}
          <div className="relative z-10 flex items-center justify-between gap-1.5 sm:gap-2 w-full mt-auto">
            <span className="shrink-0 rounded-full bg-emerald-600 px-2.5 sm:px-3.5 py-0.5 sm:py-1 text-[8px] sm:text-[11px] font-bold text-white uppercase shadow-md whitespace-nowrap">
              Beli Sekarang
            </span>
            <span className="shrink-0 text-[8px] sm:text-xs text-emerald-300 font-bold text-right hidden xs:inline-block">
              Free Ongkir
            </span>
          </div>
        </div>
      ...
      renderContent: () => (
        <div className="relative flex h-full w-full flex-col justify-between overflow-hidden bg-gradient-to-br from-[#0d2149] via-[#091533] to-[#030917] p-4 sm:p-5 text-white">
          ...
          <div className="relative z-10 flex items-center justify-between">
            ...
            <span className="rounded-full bg-sky-500/20 px-2 py-0.5 text-[9px] font-bold text-sky-300 border border-sky-500/30">
              ⚡ FAST DELIVERY
            </span>
          </div>
          ...
          <div className="relative z-10 my-auto py-1">
            ...
            <p className="mt-0.5 text-[10px] text-white/75">
              Layanan Kirim Instan & Sameday Terpercaya
            </p>
          </div>

          {/* Bottom Flex Row (Badge + Dots + Right Text) */}
          <div className="relative z-10 flex items-center justify-between gap-2 w-full mt-auto">
            <span className="shrink-0 rounded bg-sky-600 px-2.5 py-0.5 text-[9px] font-bold text-white uppercase">
              Info Selengkapnya
            </span>
            <span className="shrink-0 text-[9px] text-sky-300 font-bold text-right">
              Fast Delivery
            </span>
          </div>
        </div>
  ...
  // If custom banner 2 exists in backend data, insert it
  if (b2 && b2.imageUrl) {
    bottomCardSlides.unshift({
      id: "bottom-custom-b2",
      linkHref: "/produk",
      renderContent: () => (
        <div className="relative flex h-full w-full flex-col justify-between overflow-hidden bg-slate-900 p-4 text-white">
          <Image src={b2.imageUrl} alt={b2.title} fill className="object-cover opacity-75" />
          ...
          <div className="relative z-10 my-auto">
            <h3 className="text-base font-black text-white leading-tight">{b2.title}</h3>
            {b2.subtitle && <p className="text-xs text-white/80">{b2.subtitle}</p>}
          </div>
          <div className="relative z-10 flex items-center justify-between gap-2 w-full mt-auto">
            <span className="shrink-0 text-[9px] font-bold text-white/80">AGRES.ID</span>
            <span className="shrink-0 text-[9px] font-bold text-emerald-400">Featured</span>
          </div>
        </div>
      ),
    });
  ...
  return (
    <section className="mx-auto w-full max-w-[1260px] px-4 py-4 md:px-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3.5 sm:gap-4">
        ...
        <div
          className="relative lg:col-span-8 w-full h-[240px] sm:h-[340px] md:h-[380px] lg:h-[400px] group"
              ...
              {mainSlides.map((_, i) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all duration-300 ${
                    activeMainIndex === i ? "w-5 bg-white" : "w-2 bg-white/40 hover:bg-white/70"
                  }`}
                />
              ))}
            </div>
        ...
        {/* ── 2. TWO ROTATABLE SIDE CARDS (Right - 4 Columns on Desktop, 2 Columns Kanan-Kiri on Mobile) ───── */}
        <div className="lg:col-span-4 grid grid-cols-2 lg:grid-cols-1 gap-2 sm:gap-4 lg:h-[400px]">
          {/* Top Side Card Slider */}
          <SideCardSlider
            slides={topCardSlides}
            autoPlayMs={4200}
            className="h-[155px] xs:h-[170px] sm:h-[190px] lg:h-[192px] w-full"
          />
