...
function JasaPageContent() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`http://localhost:3001/api/services?search=${encodeURIComponent(searchQuery)}`)
      .then((r) => r.json())
  ...
  const FilterPanel = (
    <div className="space-y-6">
      <section>
        <h3 className="font-semibold text-xs uppercase tracking-widest text-[#191c1d] mb-3">Kategori Jasa</h3>
        <ul className="space-y-1">
          <li>
            <button onClick={() => setDraftFilter((p) => ({ ...p, category: "all" }))}
          ...
          {categories.map((cat) => (
            <li key={cat}>
              <button onClick={() => setDraftFilter((p) => ({ ...p, category: cat }))}
                className={`flex items-center gap-3 w-full py-2.5 pl-3 rounded-lg text-sm font-semibold transition-all ${
                  draftFilter.category === cat
                    ? "text-[#003527] bg-[#f3f4f5] border-l-4 border-[#003527]"
                    : "text-[#707974] hover:bg-[#edeeef] border-l-4 border-transparent"
                }`}>
                <span className="material-symbols-outlined text-lg">{CATEGORY_ICONS[cat] ?? "design_services"}</span>
                {cat}
              </button>
            </li>
          ))}
      ...
      <section>
        <h3 className="font-semibold text-xs uppercase tracking-widest text-[#191c1d] mb-3">Rentang Harga</h3>
        <div className="space-y-3">
          <div>
            ...
            <div className="relative">
              ...
              <input type="number" min="0" placeholder="0" value={draftFilter.priceMin}
          ...
          <div>
            ...
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-[#707974]">Rp</span>
              <input type="number" min="0" placeholder="Tidak terbatas" value={draftFilter.priceMax}
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] text-[#191c1d]">
        ...
        <aside className="hidden md:block w-60 flex-shrink-0">
          <div className="sticky top-24 bg-white rounded-xl border border-[#e1e3e4] p-5 shadow-sm">
            <div className="flex items-center justify-between mb-5">
              ...
              {activeCount > 0 && (
                <button onClick={() => { setDraftFilter(DEFAULT_FILTER); setActiveFilter(DEFAULT_FILTER); }}
                  className="text-xs text-[#ba1a1a] font-semibold hover:underline">Reset</button>
              )}
            </div>
            {FilterPanel}
            <button onClick={() => setActiveFilter({ ...draftFilter })}
        ...
        <main className="flex-grow min-w-0">
          <div className="flex items-center justify-between gap-3 mb-4">
            <div>
              ...
              <p className="text-xs text-[#707974] mt-0.5">
                {loading ? "Memuat..." : `${filtered.length} jasa tersedia`}
                {activeCount > 0 && (
            ...
            <div className="flex items-center gap-2">
              <select value={activeFilter.sortBy}
          ...
          {searchQuery && (
            <div className="mb-6 bg-white border border-[#bfc9c3]/30 rounded-xl p-4 flex items-center justify-between shadow-sm">
              <p className="text-sm text-[#404944]">
                Hasil pencarian untuk jasa: <strong className="text-[#064e3b]">"{searchQuery}"</strong>
              </p>
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1,2,3,4,5,6].map((i) => (
                <div key={i} className="bg-white rounded-xl overflow-hidden animate-pulse">
                  <div className="h-48 bg-[#edeeef]" />
                  <div className="p-5 space-y-3">
                    <div className="h-4 bg-[#edeeef] rounded w-3/4" />
                    <div className="h-3 bg-[#edeeef] rounded w-1/2" />
                    <div className="h-5 bg-[#edeeef] rounded w-1/3" />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <button onClick={() => setActiveFilter(DEFAULT_FILTER)}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((svc) => (
                <Link key={svc.id} href={`/jasa/${svc.slug}`}
      ...
      {/* Footer */}
      <footer className="bg-[#e7e8e9] w-full py-16 mt-8 border-t border-[#bfc9c3]">
    ...
    <Suspense fallback={
      <div className="min-h-screen bg-[#f8f9fa] flex items-center justify-center">
        <div className="text-center">
          <div className="h-8 w-8 animate-spin rounded-full border-4 border-[#064e3b] border-t-transparent mx-auto mb-4" />
          <p className="text-sm text-[#404944]">Memuat katalog jasa...</p>
        </div>
      </div>
    }>
      <JasaPageContent />
