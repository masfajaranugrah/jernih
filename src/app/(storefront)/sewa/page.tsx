...
function SewaPageContent() {
  const [items, setItems] = useState<ApiRentalItem[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`http://localhost:3001/api/rentals/items?search=${encodeURIComponent(searchQuery)}`)
      .then((r) => r.json())
  ...
  // Ambil kategori unik dari nama item (fallback karena RentalItem tidak punya category relation)
      ...
      <div className="max-w-[1280px] mx-auto px-4 md:px-12 pt-8 pb-20 flex flex-col md:flex-row gap-10">
        ...
        <main className="flex-grow min-w-0">
          ...
          <div className="flex items-center justify-between gap-3 mb-4">
            <div>
              ...
              <p className="text-xs text-[#707974] mt-0.5">
                {loading ? "Memuat..." : `${filtered.length} item tersedia`}
          ...
          {searchQuery && (
            <div className="mb-6 bg-white border border-[#bfc9c3]/30 rounded-xl p-4 flex items-center justify-between shadow-sm">
              <p className="text-sm text-[#404944]">
                Hasil pencarian untuk sewa: <strong className="text-[#064e3b]">"{searchQuery}"</strong>
              </p>
          ...
          {loading ? (
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div key={i} className="bg-white rounded-xl overflow-hidden animate-pulse">
                  <div className="aspect-[4/5] bg-[#edeeef]" />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filtered.map((item) => {
                // Bersihkan tag kategori dari deskripsi jika ada
      ...
      <footer className="bg-[#e7e8e9] w-full py-16 mt-8 border-t border-[#bfc9c3]">
        <div className="max-w-[1280px] mx-auto px-6 md:px-12 flex flex-col md:flex-row justify-between items-start gap-10">
          <div className="space-y-4 max-w-sm">
            ...
            <p className="text-[#707974] text-base">Platform sewa terpercaya untuk kebutuhan Anda.</p>
          </div>
          <div className="grid grid-cols-2 gap-10">
            <div className="space-y-3">
              ...
              <ul className="space-y-2 text-[#707974]">
                {["Hubungi Kami", "Syarat Sewa", "Pusat Bantuan"].map((item) => (
                  <li key={item}>
                    <a href="#" className="text-sm hover:text-[#003527] transition-colors">
                      {item}
                    </a>
                  </li>
                ))}
    ...
    <Suspense fallback={
      <div className="min-h-screen bg-[#f8f9fa] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-[#404944]">Memuat katalog sewa...</p>
        </div>
      </div>
    }>
      <SewaPageContent />
