...
function getCategory(item: ApiRentalItem) {
  return item.description?.match(/^\[cat:([^\]]+)\]/)?.[1] ?? "Sewa";
}
...
function RentalCard({ item, index }: { item: ApiRentalItem; index?: number }) {
  const badgeText = getBadgeText(item, index);
  const ratingValue = item.rating && item.rating > 0 ? item.rating.toFixed(1) : (4.6 + ((index || 0) % 4) * 0.1).toFixed(1);
  const rentCount = (item as { rentCount?: number }).rentCount ?? ((index || 0) * 4 + 7);
  ...
  return (
    <div className="group relative flex flex-col rounded-[28px] border border-neutral-100 bg-white p-3.5 shadow-xs sm:rounded-[32px] sm:p-4 hover:shadow-md transition-all h-auto">
      <Link href={`/sewa/${item.slug}`} className="relative -mx-3.5 -mt-3.5 block aspect-[4/3] w-[calc(100%+1.75rem)] shrink-0 overflow-hidden rounded-t-[27px] bg-[#f2f4f7] sm:-mx-4 sm:-mt-4 sm:w-[calc(100%+2rem)] sm:rounded-t-[31px]">
        {badgeText && (
      ...
      <div className="mt-2.5 flex flex-col items-start px-0.5">
        ...
        <div className="mt-2 pt-1 flex flex-col gap-1">
          ...
          <div className="mt-1 flex flex-col items-start">
            ...
            <span className="text-xs font-semibold text-neutral-400">/hari</span>
          </div>
        </div>
      </div>
    </div>
...
export default function SewaPageClient({ items, categories, resolvedSearch }: Props) {
  ...
  const searchSuggestions = useMemo(() => {
    ...
    return items
      .filter((item) => item.name.toLowerCase().includes(q) || getCategory(item).toLowerCase().includes(q))
      .slice(0, 5);
  }, [items, searchQuery]);
    ...
    return items
      .filter((item) => {
        if (searchQuery.trim()) {
          ...
          const matchCat = getCategory(item).toLowerCase().includes(q);
          if (!matchName && !matchCat) return false;
        }

        if (activeFilter.category !== "all" && activeFilter.category !== "Semua" && getCategory(item) !== activeFilter.category) {
          return false;
        }
      ...
      .sort((a, b) => {
        if (activeFilter.sortBy === "harga_asc") return parseFloat(String(a.pricePerDay)) - parseFloat(String(b.pricePerDay));
        if (activeFilter.sortBy === "harga_desc") return parseFloat(String(b.pricePerDay)) - parseFloat(String(a.pricePerDay));
  ...
  }, [items, filteredAll.length, searchQuery]);
        ...
        <div className="flex flex-col gap-4 mb-5">
          <div className="flex items-center gap-2.5 overflow-x-auto no-scrollbar py-1">
            {tabCategories.map((tab) => {
              ...
              const count = categoryCounts[tab] || (tab === "Semua" ? items.length : 0);
          ...
          <div className="relative flex items-center gap-3" ref={searchContainerRef}>
            <div className="relative flex-1 rounded-[22px] bg-[#edf0f4] hover:bg-[#e5e9ef] focus-within:bg-white focus-within:ring-2 focus-within:ring-blue-600 border border-transparent transition-all flex items-center px-4 py-3 shadow-inner">
              ...
              <input
                ...
                placeholder="Cari item sewa atau kategori..."
                      ...
                      <li key={item.id}>
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[11px] font-semibold text-neutral-400">{getCategory(item)}</p>
                          </div>
                          <span className="text-xs font-extrabold text-blue-600 shrink-0">{formatRupiah(item.pricePerDay)}</span>
                        </Link>
                      </li>
                    ))}
                  </ul>
          ...
          <aside className="hidden w-64 shrink-0 md:block xl:w-72">
            <div className="sticky top-24 rounded-3xl border border-neutral-200/80 bg-white p-5 shadow-xs">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-extrabold text-sm text-neutral-900">Filter Sewa</h3>
                {activeCount > 0 && (
                  <button
                    type="button"
                    onClick={() => {
          ...
          <main className="flex-1 min-w-0">
            {filteredAll.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center bg-white rounded-3xl border border-neutral-200/60 p-8 shadow-2xs">
                ...
                <p className="font-extrabold text-neutral-800 text-base sm:text-lg">
                  {items.length === 0 ? "Belum ada item sewa" : "Item sewa tidak ditemukan"}
                </p>
                ...
                {recommendedItems.length > 0 && (
                  <div className="mt-8 w-full text-left">
                    <p className="text-xs font-extrabold uppercase tracking-wider text-neutral-400 mb-4 text-center">
                      Referensi item sewa lain yang mungkin Anda cari:
                    </p>
                    <div className="grid grid-cols-2 gap-3.5 sm:grid-cols-4 items-start">
                      {recommendedItems.map((item, idx) => (
                        <RentalCard key={item.id} item={item} index={idx} />
                      ))}
                    </div>
                  </div>
                )}
        ...
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-3xl md:hidden max-h-[85vh] flex flex-col shadow-2xl">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-200">
              <h2 className="font-extrabold text-base text-neutral-900">Filter Sewa</h2>
