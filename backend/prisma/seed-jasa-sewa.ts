// prisma/seed-jasa-sewa.ts
// Seed data dummy: 20 jasa + 20 sewa + kategori jasa pendukung. Idempoten (upsert by slug / name).
// Jalankan:  npx ts-node backend/prisma/seed-jasa-sewa.ts
import { PrismaClient } from '@prisma/client';

const prisma = new PrismaClient();

// Kategori jasa (nama harus cocok dengan CATEGORY_ICONS di JasaPageClient bila ada)
...
interface SeedService {
  name: string;
  slug: string;
  categoryName: string; // canonical lihat JASA_CATEGORIES
  priceFrom: number;
  unit: string;
  rating: number;
  description: string;
...
interface SeedRental {
  name: string;
  slug: string;
  cat: string; // kategori disimpan sebagai prefix "[cat:...]" di description
  pricePerDay: number;
  deposit: number;
  rating: number;
  description: string;
...
async function main() {
  ...
  for (let i = 0; i < rentals.length; i++) {
    const r = rentals[i];
    const existing = await prisma.rentalItem.findUnique({ where: { slug: r.slug }, select: { id: true } });
    const data = {
      name: r.name,
    ...
    if (existing) {
      await prisma.rentalItem.update({ where: { id: existing.id }, data });
      rUpdated++;
    } else {
      await prisma.rentalItem.create({ data: { slug: r.slug, ...data } });
      rCreated++;
    }
  }
